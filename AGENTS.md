...
- **Abertura:** quando eu cumprimentar pra começar ("bom dia", "boa tarde", "boa noite", "tudo bem Codex"), rode a skill `session-start` — leia `plans/ESTADO.md` (e o último handoff se precisar do raciocínio) e me dê o recap antes de tocar em código.
...
- **Estrutura de `plans/` — cada arquivo é dono de um dado, sem sobreposição:**
  ...
  - `artefatos/R-NN-{slug}.html` — mockups gerados pelo Codex. Cabeçalho obrigatório em comentário HTML (item, rota alvo, componente alvo, data, status). **Nunca lidos pro contexto** — abrem no browser pra comparar com a tela real. Os tokens que a implementação segue ficam na spec, em texto.
...
- **Precedência — quando duas fontes discordam, esta é a ordem:**
  ...
  3. **`AGENTS.md`** — as regras permanentes do projeto
