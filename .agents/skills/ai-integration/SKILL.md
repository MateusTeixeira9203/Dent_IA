...
export async function analyzeText(text: string) {
  ...
  const { object } = await generateObject({
    model: anthropic('Codex-haiku-4-5'),  // Haiku para tarefas simples/baratas
...
| Caso | Modelo | Por quê |
|---|---|---|
| Resposta simples, classificação | `Codex-haiku-4-5` | Barato, rápido |
| Chat, análise, geração | `Codex-sonnet-4-5` | Equilíbrio custo/qualidade |
| Raciocínio complexo, código difícil | `Codex-opus-4-8` | Máxima capacidade |
| Estruturação/extração de dados | `Codex-haiku-4-5` + `generateObject` | Schema garante formato |
