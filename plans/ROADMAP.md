...
| **R-72** | 🗑️ **Matar `/consulta` de vez** — decisão dele 07/08; reforçada a tarde: `/consulta` é legado, foco total é o Meu dia | ✅ **fechado 07/08, testado ao vivo por ele, commitado e no ar** (`9e13626`). F1: `actions.ts`→`server/patients/registro-actions.ts`, `tooth-group-list.tsx`→`components/odontograma/tooth-group-list.tsx` (7 imports atualizados). F2: rota `/consulta/[agendamentoId]` e `/consulta/demo` **apagadas** (9 arquivos); os 6 botões "Iniciar consulta"/"Continuar atendimento" (Agenda ×2, ficha do paciente, Dex, Atender agora, Rail do Meu dia) agora vão pra `/dashboard/meu-dia?ag={id}` — o Rail perdeu o link (redundante, já está no Meu dia) e a `podeAtender` que só ele usava. Onboarding: passo "demo" saiu dos 2 lados (`onboarding-progress.ts` 5→4 passos, `onboarding-client.tsx` etapa 'aha' — já estava inalcançável desde uma Fase 1 anterior, confirmado no código antes de apagar). `ModoConsultaLoader` (órfão) apagado. `DexGuide`/`dex-mascot.tsx` já eram Fase-1-desativados (apontavam pra demo em texto) — deixados como débito pré-existente, fora do escopo | G |
| [R-46h](specs/R-46h-orcamento-no-meu-dia.md) | **Botão de orçamento no Meu dia** — modal picker lista fichas em aberto do paciente, escolhe uma, gera só dela (mesma regra do R-59); acessível também da aba Histórico | 🟡 **codado e commitado 08/08** (`fb4d031`) — F1 (hook extraído) + F2 (botão por-visita no Histórico) + F3 (picker geral no rodapé). Verificado por mim no Brave: os 2 pontos de entrada abrem com preço certo. **Ele ainda não testou pessoalmente** | M |
...
| **R-71** | 🔧 Polimento pós-auditoria — Base UI `nativeButton` warning (`not-found.tsx`/`error.tsx`) + Agenda com janela de hora fixa `7h-20h` em `day-view.tsx`/`week-view.tsx` (agendamento de madrugada OU depois das 20h fica inalcançável, os 2 lados) | ⏳ achados 1+2 da [auditoria pré-produção](../auditorias/2026-08-07-pre-producao.md) 07/08. Baixo risco nos 2, nenhum bloqueia o push. Agenda: porta `janelaDeHoras` que o R-64 já tem em `retorno-semana-grid.tsx:41`, não constrói de novo. **Achado 3 (observação sem "ver mais") absorvido pelo [R-77](specs/R-77-historico-scroll-observacao.md) 08/08** | P |
| [R-76](specs/R-76-salvar-e-passar.md) | **Salvar e passar** — botão "Salvar" do Registrar volta a avançar sozinho pro próximo paciente do rail | 🟡 **codado e commitado 08/08** (`7602659`) — reverte C2/P7 (03/08), decisão dele. `podeAtender`/`avancarProximo` recriadas em `meu-dia-client.tsx`. Verificado por mim no Brave com escrita real: salvei uma visita e caiu no estado de fim de dia (não tinha próximo elegível), sem reabrir o 1º paciente escondido. **Ele ainda não testou pessoalmente** | P |
| [R-77](specs/R-77-historico-scroll-observacao.md) | **Histórico: scroll no modo prévia + observação expansível** — teto de altura sempre (não só expandido) + `TextoExpansivel` extraído de `TextoVisita`, reusado na observação do `RegistroCard` | 🟡 **codado e commitado 08/08** (`34a33e5`) — absorve o achado 3 do R-71. Verificado por mim no Brave: observação com aspas curvas + `line-clamp-2`, scroll ativo nos 2 modos. **Ele ainda não testou pessoalmente** | P |
| **R-80** | 🐛 **Orçamento podia puxar procedimento indicado por OUTRO dentista** — achado ao vivo por ele 08/08 testando o R-46h. `carregarFichasAgregado`/`abrirOrcamentoParaFicha` filtravam só por paciente+clínica; o histórico do paciente é compartilhado da clínica, então o picker do Meu dia listava ficha de colega | 🟡 **codado, verificado no Brave e commitado 08/08** (`366cd64` base + trava embutida no `fb4d031` do R-46h — sintaticamente dependentes, não deram pra separar sem quebrar o build em commit intermediário) — `restringirAoMeuDentista` (default false: tela do paciente mantém o agregado compartilhado de propósito, decisão 07/08); Meu dia passa `true`. Botão do Histórico também some pra ficha de outro dentista (`MeuDiaVisita.dentistaId`, novo). Defesa em profundidade: a query recusa mesmo se o botão vazar. **Ele ainda não testou pessoalmente** | P |
| [R-78](_arquivo/specs/R-78-coluna-direita-ficha-viva.md) | **Meu dia orientado a fluxo** (redesign completo) — odontograma vira espelho, lista única "Nesta ficha" com status clicável, perfil do dente como ocupante da direita, gavetas, "ler grande", rótulo do rodapé | ✅ **fechado 08/08, verificado ao vivo por ele e no ar** (`a12a319`) — F0-F5 completos. Detalhe em spec §5 (arquivada) | G |
| **R-79** | 🔧 **Ficha editada não deixa rastro** — `salvar-ficha.ts` grava só `updated_at` no ramo de update: nem quem editou, nem o quê mudou. Prontuário pode ser reescrito sem ninguém saber | ⏳ achado 08/08 no debate do R-78. **Não é regressão** — vale pra toda edição de ficha salva hoje (`FichasTab`), só nunca tinha sido nomeado. CFO pede rastreabilidade. Sem spec ainda | M |
| **R-82** | 🐛🔴 **Campo mágico trava a aba** — "Maximum update depth exceeded" em `captura-livre-card.tsx` (loop de `setState`), reproduzido **5×** ao vivo testando o R-78 F1/⤢. **Padrão real (não é só "reorganizar 2×" como pensei no 1º achado):** acontece com o campo mágico ABERTO enquanto outra coisa no resto da tela re-renderiza — reorganizar de novo, mexer nos cards de "Nesta ficha", clicar ⤢. Uma vez travou a aba de verdade (só navegar recuperou), as outras 3× o React conteve sozinho (sem travar, só o erro no console) — parece limiar de quantidade de re-render, não uma ação específica. Não reproduz no load nem com o campo mágico fechado. **Não é regressão de hoje** — não toquei em `captura-livre-card.tsx`/`useCapturaLivre.ts`/`campo-magico-meu-dia.tsx` em nenhuma sessão, e o 1º arquivo já estava no meio de mudanças não commitadas do R-75. Suspeito nº1: algo no ciclo de `texto`/`detectDebounceRef`/`anexarTexto` em `useCapturaLivre.ts` reagindo a cada re-render do pai, não só a mudança real de valor. **Prioridade alta** — é o coração do produto (Dex/campo mágico), trava peça central do fluxo de consulta. Sem spec, sem repro mínimo isolado ainda | G |
| **R-83** | 🔧 **Gerar orçamento não enxergava o rascunho atual** — achado ao vivo por ele 08/08 testando o R-78. `abrirPickerFichasAbertas` só buscava fichas já SALVAS no banco; salvar antes empurra pro próximo paciente (R-76), então não dava pra orçar o que acabou de ditar sem sair da tela dele | 🟡 **codado e verificado no Brave 08/08** — `abrirPickerFichasAbertas(eventosRascunho?)` junta o rascunho (convertido via `draftParaEventoOrc`, mesmo padrão de `paraCard`) com o agregado do banco e pula direto pra etapa 'itens'; sem rascunho, comportamento intacto (etapa 'selecionar' como antes). Testado: Canal indicado no dente 45 (rascunho) apareceu junto com 3 itens de fichas já salvas, R$ 3.600 total. **Ele ainda não testou pessoalmente** | P |
| [R-84](specs/R-84-nesta-ficha-novo-vs-antigo.md) | **"Nesta ficha" distingue novo × antigo, e o orçamento só vê o novo** — pendência antiga concluída hoje fica na lista **marcada** (e editável), mas sai do orçamento; o merge do agregado do R-83 é cortado | 🟡 **codado e testado ao vivo 08/08, gate `/auditar` passou — falta commit/push.** Nasce do fluxo real ("avaliação gera o orçamento, consultas seguintes executam"): `indicado` quer dizer *já vendido, não executado* — então o agregado re-orçava o que já tinha sido fechado, e `orcamento_itens` não tem vínculo com `odontograma_eventos` pra saber. Discriminador derivado de `boca` (R-61), zero migration. **Gates G1-G9 confirmados ao vivo** (paciente real com pendências de verdade, Salvar real conferido no banco — `ficha_id` não muda, as 2 datas aparecem no Histórico). `typescript-reviewer` achou e eu corrigi 1 regressão real antes de fechar: `podeTrocarFicha` perdia o `← Voltar` depois de escolher uma ficha específica numa lista de 2+ — corrigido e reverificado ao vivo (round-trip completo). `ux-reviewer` aprovou sem CRITICAL/HIGH | M |
| **R-81** | 👥 **Secretária registra PELO dentista** — fluxo real do consultório relatado por ele 08/08 ("já vi muitas vezes o dentista mandar a secretária registrar"), hoje **bloqueado**: `meu-dia/page.tsx:24` redireciona secretaria (agendamento é silo por `dentista_id`, então "meu dia" dela seria sempre vazio) | ⏳ achado 08/08 no debate do R-78. **Armadilha conhecida:** sem `dentistaId` explícito o registro nasce preso ao perfil dela, não do dentista que atendeu — mesma classe achada 3× em 14/07 (`project_rls_silo_secretaria_dentista_id`). Precisa de: seletor de "dia de quem" + `dentistaId` explícito em toda escrita + gate de 2 contas. Sem spec. **Possivelmente mais valioso pro dia a dia que o R-78 inteiro.** **Esclarecido 08/08:** dentista fica fisicamente presente e dita cada comando em tempo real — ela só anota/executa por ele, não decide sozinha. Muda o que "confuso" significa pro design (não precisa suportar decisão independente de não-dentista). **Ordem decidida: R-78 primeiro** | G |
| **R-73** | 🐛 **Rail do Meu dia engolia clique** — "+Encaixe" e trocar de paciente não respondiam, em prod e local, achado por ele 07/08 | ✅ **fechado 07/08, verificado ao vivo por ele e no ar** (`6edc60f`). 3 bugs reais em `rail.tsx`: ref de arrasto nunca zerava depois de clique normal (ficava presa, suprimia todo clique seguinte); limiar de 5px sozinho classificava tremor de clique como arrasto (exige também tempo mínimo); o `click` nativo mirava em quem estivesse na coordenada no `pointerup`, não em quem estava lá no `pointerdown` — se o rail refluía no meio do gesto, o clique acertava o elemento errado sem erro nenhum. Fix final: resolve o alvo no `pointerdown`, dispara no `pointerup` por esse alvo guardado | P |
| **R-74** | Trocar status do atendimento manualmente no Meu dia — pedido dele 07/08 (Salvar nem sempre marca "atendido": ficha rápida fora do fluxo de agendamento, ou clique errado a corrigir) | ✅ **fechado 07/08, verificado ao vivo por ele e no ar** (`6edc60f`) — seletor no card selecionado do Rail, reusa `atualizarStatusAgendamento` (mesma escrita da Agenda, 2º ponto de entrada, zero schema novo) | P |
| **R-75** | 🐛 **Dex marcava "realizado" em documento importado que não era relato ao vivo** — dentista subiu um arquivo com histórico, Dex leu verbo no passado como "feito hoje" e marcou tudo | 🟡 **codado e verificado por eval 07/08** — `modo:'exame_inicial'` (existia morto no código, nunca usado) agora liga sozinho quando o texto vem de upload de documento (nunca de áudio — narração continua "consulta"). Prompt ganha regra condicional: sem confirmação explícita de conclusão, status vira `indicado`. **Eval antes/depois**: 15/16→15/16 atual (1 erro 500 transitório em cada rodada, caso diferente, sem padrão), 0 inventados nas duas, os 2 casos novos (ambíguo→indicado, explícito→mantém realizado) PASS. Não testado no upload real da UI ainda | G |
