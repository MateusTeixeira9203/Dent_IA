...
| ID | Item | Estado | Peso |
|---|---|---|---|
| 🔵 [**R-108**](specs/R-108-ficha-tratamento.md) | **Ficha = documento de tratamento** — modelo (`ficha_evolucoes` + nome) e a ficha nova, em leitura. **Nenhum caminho de escrita muda** | 🟡 **codado e gates confirmados ao vivo por ele 13/08** (light+dark, legado intacto, nome derivado, timeline de evoluções — paciente "tes"/"teste" em Teste01). Migration 141 no ar (174/174 backfill). **G3 (2 contas) represado.** Sem commit ainda. Bug de origem (evento na ficha errada) **não resolvido aqui** — depende do R-108b | G |
| [**R-108b**](specs/R-108b-roteamento-da-visita.md) | **Roteamento da visita** — pendência volta pra sua ficha sem perguntar; só procedimento novo escolhe destino; seletor cabe no cabeçalho "Nesta ficha" que já existe (custo vertical zero) | ⏳ **spec aprovada 13/08**, zero código. É quem mata o bug de origem (evento na ficha errada). É a única fatia que muda a rota de escrita com paciente real — **retomar em Opus**, não Sonnet | M |
