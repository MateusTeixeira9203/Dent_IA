...
| **R-107** | Remodelagem da barra do Meu dia + perfil do dente — debate 12-13/08 com ele | **a** — [spec](specs/R-107a-barra-meu-dia.md) 🟡 codada e testada ao vivo 13/08 (localhost, paciente "teste"): tira status/observação globais, traz chip de Profilaxia/Clareamento (porta de `FichasTab.tsx` via `rotina-boca.ts` novo), renomeia "+texto da visita"→"+Observação". Todos os 8 gates confirmados ao vivo, zero erro de console, zero migration. **Não commitada.** **b** — [spec](specs/R-107b-perfil-do-dente.md) 🟡 codada e testada ao vivo 13/08, **migration 139 APLICADA em produção** (`'outro'` no CHECK, conferido no schema): tira chip Incluso (tipo fica), busca livre no painel (reusa `casarProcedimentoLocal`), tipo genérico `outro`, dente ausente via `exodontia`+`origem:'preexistente'` + fix do dedup da Extração. G1-G8 confirmados (G5 medido por DOM: os 2 eventos coexistem e removem independente; G6 com escrita real, item de teste apagado depois). Achado registrado: a busca não é type-ahead — só casa nome quase inteiro. **Não commitada.** **c** — [spec](specs/R-107c-altura-estavel-perfil-dente.md) 🟡 codada e testada ao vivo 13/08: `min-h-[308px]` no card da direita do Meu dia. Medido antes/depois: dente vazio 139px→308px, tabela de especialidade aberta continua 570px (nada cortado). Zero erro de console. **Não commitada**. **d** — [spec](specs/R-107d-lote-multidente.md) 🟡 codada e testada ao vivo 13/08: chips de lote (N dentes → 1 procedimento pra todos), Restauração pede face antes, Dente ausente e busca livre em lote reusando o R-107b. G1-G8 confirmados (G4 testado com 4 dentes sem colidir com Restauração já registrada nos mesmos 2). Achado registrado: montar a seleção ainda exige "voltar à boca" entre cada clique — a faixa resolve aplicar, não montar. Zero erro de console. **Não commitada** | G |
