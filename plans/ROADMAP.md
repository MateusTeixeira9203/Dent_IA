...
| [**R-109**](specs/R-109-registro-na-ficha.md) | **Registro na ficha** — lote multidente + Modo multidente portados do Meu dia, chips locais ligados, trilho duplo morre na escrita | ⏳ **spec aprovada 13/08**, zero código. Independente do R-108, sobe em qualquer ordem, cabe em Sonnet | M |
| **R-110** | **Horário do dentista vale de verdade na agenda** — `criarAgendamento` valida conflito de dentista, de paciente e bloqueio pessoal, mas **nunca olha `horarios_disponiveis`**. A secretária marca fora do expediente sem aviso | ⏳ pedido dele 13/08, **sem spec**. 5 decisões abertas: (1) dentista sem grade cadastrada ficaria trancado; (2) tem override, como o `forcarConflitoDentista`?; (3) vale pro dentista ou só pra secretária; (4) editar/arrastar usa outro caminho; (5) agendamentos já marcados fora do horário | M |
