...
| [R-78](specs/R-78-coluna-direita-ficha-viva.md) | **Meu dia orientado a fluxo** (redesign completo — não é mais só "coluna direita") — odontograma vira espelho (~555px), lista única "Nesta ficha" com status clicável, perfil do dente como ocupante da direita, "A fazer" e Histórico viram gaveta | 🔵 **ativo — F0+F1+F2 codados e verificados por mim 08/08** (dado real — Marcos, Brave). Grid vira fluxo vertical, `RegistrarPainel` vira hook, espelho a `zoom:.68` (G7/G9 medidos). Toque no dente abre **histórico** (não o editor direto — achado dele corrigiu o F0, G12 confirmado). "Nesta ficha" é `RegistroCard` de verdade (pill clicável, observação editável, "✓ tudo feito", remover — `corpoEspecialidadeEditavel` extraído de `FichasTab.tsx`, zero duplicação nova). Tabela de especialidade abre pelo ⤢ (`onAbrirGrande`, novo) **dentro do card do perfil do dente** (não full-width separado — 2ª correção a partir de print dele, a 1ª versão ficava sem fundo). **Ele testou F0-F2 pessoalmente 08/08** — achou bug (overlap data/tipo no histórico do dente, corrigido) e pediu animação leve na expansão/retração (gavetas + corpo de especialidade), que causou um "flick" corrigido em seguida (`layout`/FLIP em vez de animar `height`, `mode="wait"`). **F3 confirmado sem gap, F4 (ler grande do texto de evolução) e F5 (rótulo `✓ 1 ficha hoje`) codados e verificados por mim 08/08** — item inteiro codado, falta só ele aprovar e eu commitar. Detalhe em spec §5 | G |
