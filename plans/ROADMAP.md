...
| [R-46h](specs/R-46h-orcamento-no-meu-dia.md) | **Botão de orçamento no Meu dia** — modal picker lista fichas em aberto do paciente, escolhe uma, gera só dela (mesma regra do R-59); acessível também da aba Histórico | ⏳ **spec aprovada 08/08** — §4 decidido: os dois pontos de entrada entram (Histórico por-visita + rodapé do Registrar, picker geral). Escopo mudou desde 02/08: não é mais CTA preso ao save, é ponto de entrada independente. Motor quase todo já existe em `paciente-detail-client.tsx` (`carregarFichasAgregado`/`abrirOrcamentoParaFicha`/passo `selecionar`) — o trabalho real é extrair pra `useOrcamentoModal` compartilhado, não escrever de novo | M |
...
| [R-76](specs/R-76-salvar-e-passar.md) | **Salvar e passar** — botão "Salvar" do Registrar volta a avançar sozinho pro próximo paciente do rail | ⏳ **spec aprovada 08/08** — reverte C2/P7 (03/08), decisão dele. `podeAtender` (apagada no R-72 ontem) recriada local em `meu-dia-client.tsx`; `avancarProximo()` idêntica à versão pré-03/08. Zero código ainda | P |
| [R-77](specs/R-77-historico-scroll-observacao.md) | **Histórico: scroll no modo prévia + observação expansível** — teto de altura sempre (não só expandido) + `TextoExpansivel` extraído de `TextoVisita`, reusado na observação do `RegistroCard` | ⏳ **spec aprovada 08/08** — absorve o achado 3 do R-71. Tabela de especialidade expansível **já existe** (confirmado por grep, `defaultOpen` nunca é `true`) — não é trabalho novo. Zero código ainda | P |
| **R-80** | 🐛 **Orçamento podia puxar procedimento indicado por OUTRO dentista** — achado ao vivo por ele 08/08 testando o R-46h. `carregarFichasAgregado`/`abrirOrcamentoParaFicha` filtravam só por paciente+clínica; o histórico do paciente é compartilhado da clínica, então o picker do Meu dia listava ficha de colega | 🟡 **codado e verificado no Brave 08/08** — `restringirAoMeuDentista` (default false: tela do paciente mantém o agregado compartilhado de propósito, decisão 07/08); Meu dia passa `true`. Botão do Histórico também some pra ficha de outro dentista (`MeuDiaVisita.dentistaId`, novo). Defesa em profundidade: a query recusa mesmo se o botão vazar. **Não commitado** | P |
