...
> **12/08, pós-R-103a:** matcher local do campo mágico agora casa plural — `casaLabel` em
> `casar-procedimento-local.ts` cobre os 2 padrões achados (-ão→-ões, -al→-ais); os 6 rótulos
> que quebravam (Restauração, Extração, Canal, Lesão periapical, Coroa total, Exame
> periodontal) casam. Verificado em harness Node isolado (28/28). **Codado, não commitado.**
> Status realizado/indicado errado na extração por IA virou **R-106** — exige eval
> antes/depois (regra do CLAUDE.md), não investigado a fundo ainda. Detalhe no
> [handoff](handoffs/handoff-2026-08-12-1323.md).
...
> **Fila:** 28⏳ · **🟡 codado/no ar sem verificação pessoal dele:** 40 · **💡 ideia sem spec:** 3 ·
...
| ID | Item | Estado | Peso |
|---|---|---|---|
| [**R-108**](specs/R-108-ficha-tratamento.md) | **Ficha = documento de tratamento** — modelo (`ficha_evolucoes` + nome) e a ficha nova, em leitura. **Nenhum caminho de escrita muda** | 🟡 **no ar 13/08** (commitado e pushado). Gates confirmados ao vivo por ele: light+dark, legado intacto, nome derivado, timeline de evoluções. Migration 141 no ar (174/174 backfill). **G3 (2 contas) represado.** Bug de origem foi resolvido pelo R-108b, não aqui | G |
| 🔵 [**R-108b**](specs/R-108b-roteamento-da-visita.md) | **Roteamento da visita** — pendência volta pra sua ficha sem perguntar; só procedimento novo escolhe destino, com seletor pré-marcado dentro do card "Nesta ficha" | 🟡 **no ar 13/08.** Migration 142 (`p_sincronizar`) aplicada e conferida no schema. Provado ao vivo na Teste01: **G4** (concluídos permanecem na origem), G6, G11 (não-destruição), G12, G13 (status derivado do conteúdo — nasce aberta, fecha sozinha), G10. **Não rodaram: G3 ("absorver"), G7, G9, G8.** A spec teve o §4 emendado antes do código: o desenho aprovado apagava evento e sobrescrevia ficha de paciente real | M |
| **R-110** | **Horário do dentista vale de verdade na agenda** — hoje `criarAgendamento` valida conflito de dentista, de paciente e bloqueio pessoal, mas **nunca olha `horarios_disponiveis`**: quem lê a grade é só o WhatsApp e o "Marcar retorno". A secretária marca fora do expediente sem nenhum aviso | ⏳ pedido dele 13/08, **sem spec**. 5 decisões abertas antes de codar: (1) dentista **sem grade cadastrada** fica trancado — `livres: []` bloquearia tudo no dia do deploy; (2) tem override como o `forcarConflitoDentista`? bloquear sem ele contradiz a decisão dele de 21/07 (*"a recepção precisa dessa liberdade"*); (3) vale pro dentista também ou só pra secretária; (4) editar/arrastar agendamento usa outro caminho — sem cobrir, o furo continua; (5) o que acontece com agendamentos já marcados fora do horário | M |
| [**R-109**](specs/R-109-registro-na-ficha.md) | **Registro na ficha** — lote multidente + Modo multidente portados do Meu dia, chips locais do campo mágico ligados, e o trilho duplo (`selectedTeeth`/`teethNotes`) morre na escrita | ⏳ **spec aprovada 13/08**, zero código. **Independente do R-108** — sobe em qualquer ordem, pode ir em Sonnet. Levantamento mostrou que 2 dos 3 caminhos (1 dente, boca inteira) **já estão na ficha**; falta o lote e 2 props | M |
...
| **R-106** | 🐛 Status realizado/indicado sai errado na extração por IA (`/api/dex/formatar-evolucao`) — mesma categoria clínica ("achado", ex. fratura/lesão periapical) sai com status diferente no mesmo relato | ⏳ achado 12/08, testando os pontuais do campo mágico. Não investigado a fundo — mexer no prompt exige eval antes/depois (regra do CLAUDE.md) | M |
| 🟡 [**R-107**](specs/R-107a-barra-meu-dia.md) | Remodelagem da barra do Meu dia + perfil do dente — debate 12-13/08 com ele | **a** — [spec](specs/R-107a-barra-meu-dia.md) 🟡 codada, testada ao vivo e **commitada 13/08** (`c32ee24`): tira status/observação globais, traz chip de Profilaxia/Clareamento (porta de `FichasTab.tsx` via `rotina-boca.ts` novo), renomeia "+texto da visita"→"+Observação". 8/8 gates ao vivo, zero erro de console, zero migration. **b** — [spec](specs/R-107b-perfil-do-dente.md) 🟡 codada, testada ao vivo e commitada, **migration 139 aplicada em produção** (`'outro'` no CHECK): tira chip Incluso (tipo fica), busca livre no painel (reusa `casarProcedimentoLocal`), tipo genérico `outro`, dente ausente via `exodontia`+`origem:'preexistente'` + fix do dedup da Extração. G1-G8 confirmados, G6 com escrita real (item de teste apagado depois). Achado: a busca não é type-ahead — só casa nome quase inteiro. **c** — [spec](specs/R-107c-altura-estavel-perfil-dente.md) 🟡 codada, testada e commitada: `min-h-[308px]` no card da direita — dente vazio 139px→308px, tabela de especialidade continua livre (570px). **d** — [spec](specs/R-107d-lote-multidente.md) 🟡 codada, testada e commitada: chips de lote, Restauração pede face antes (checado em produção — quase metade das fichas 4+ dentes tem face diferente por dente), Dente ausente/busca livre em lote. **Adendo: "Modo multidente"** (toggle que impede o clique de abrir o histórico, resolve o "clica-fecha-clica-fecha") — codado e testado, **posição ainda em aberto** (ver ESTADO.md). 5 commits locais, **sem push** | G |
...
| [**R-103**](specs/R-103-painel-do-dex.md) | **Painel do Dex** — modal central de 3 colunas: pendências · números do negócio · central de atualização | **a ✅ no ar e verificado por ele 12/08** (`b427391`) — destravado, mock fora, casca de 3 colunas, sino apagado. Spec arquivada em [`_arquivo/specs/R-103a-destravar-o-dex.md`](_arquivo/specs/R-103a-destravar-o-dex.md). **b** — [spec](specs/R-103b-pendencias-do-dex.md) 🟡 codada 12/08, **commitada 13/08** (`88d966f`): rota `/api/dex/retencao`, `classificarRetencao` (12/12 fixtures), testado ao vivo em Teste01 sem erro. **c** — [spec](specs/R-103c-o-mes-do-dex.md) 🟡 codada 12/08, **commitada 13/08**: rota `/api/dex/mes`, coluna "Hoje e a semana"→"O mês" (3 números, sem "recorrentes" — cortado por decisão dele), 7/7 fixtures, testado ao vivo em Império (6 atendimentos, +200%). **G3/G4/G5/G6/G8 pendentes** nos dois (b e c) — 2 contas reais, secretária, protético, design-review. Sem push ainda. Falta ainda: R-104 curso. **Absorve o R-26** | G |
...
| [R-26](ROADMAP.md) | Dex vira hub de notificações operacionais — faltosos sem retorno | ✂️ **absorvido pelo [R-103](specs/R-103-painel-do-dex.md) em 11/08** — a pergunta que ele deixou aberta ("o que é faltou e não voltou") virou o A1 do master | — |
