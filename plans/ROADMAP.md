...
> **Fila:** 19⏳ · **🟡 codado/no ar sem verificação pessoal dele:** 35 · **💡 ideia sem spec:** 3 ·
> **Concluídos:** 29 · **Congelado:** 3 · **Cortado:** 10
...
| [R-62](specs/R-62-campo-magico-entrada-unica.md) | Campo mágico vira entrada única — match local mata a disclosure "Registrar sem IA" | 🟡 fechada 05/08 no que é testável por automação. **G10/I4 (voz) exige microfone real** | M |
...
| [R-76](specs/R-76-salvar-e-passar.md) | Salvar e passar — "Salvar" volta a avançar sozinho pro próximo paciente do rail | 🟡 codado e commitado 08/08 (`7602659`), verificado com escrita real. **Ele ainda não testou pessoalmente** | P |
...
| **R-82** | 🐛 Campo mágico trava a aba — "Maximum update depth exceeded" em `captura-livre-card.tsx`, reproduzido 5× ao vivo testando o R-78 | 🟡 **causa achada e corrigida 08/08** (`16a45bb`): `anexarTexto` nascia objeto novo a cada render, alimentando o efeito de `captura-livre-card.tsx` sem mudança real. Memoizado — regressão ao vivo sem nenhum "Maximum update depth". **Não fechei o loop**: o caminho só existe de verdade depois de anexar documento real (Browser pane não faz upload) — precisa do teste dele | G |
...
| **R-75** | 🐛 Dex marcava "realizado" em documento importado que não era relato ao vivo | 🟡 **codado, eval reconfirmado e commitado 08/08** (`a5519eb`) — `modo:'exame_inicial'` liga sozinho em upload de documento, nunca em áudio. Eval 2×: ATUAL 15/16 sem regressão (falha é timeout de infra); caso "explícito mantém realizado" ~80% em 5 chamadas — quando erra, erra pro lado seguro. **Não testado no upload real da UI** | G |
...
| [R-28](specs/R-28-pagamento-fecha-sem-duplicar.md) | Pagamento: grava quem registrou + fecha parcela sem duplicar recebimento | 🟡 partes 1+2 verificadas na Teste01, falta confirmar em prod. **Parte 3 (saldo fantasma) não implementada** — 9 orçamentos reais em produção com `total_pago ≥ valor_devido` e parcela `pendente` aberta (cresceu de 2 pra 9 desde 31/07) | M |
