...
| R-05 | 🔵 Ortodontia: lançamento e edição manual — **codado + verificado em localhost, aguardando deploy** | `OrtoForm` montado no FichasTab (corpo editável da evolução), com adicionar/remover; caminho manual sem voz + correção da arcada errada da IA. `orto_manutencao` já era lido/salvo — faltava a entrada. Gates: typecheck+lint+build ✅; funcional via harness Playwright logado (rota demo, sem write em prod) **18/18 light+dark ✅** (render→adicionar→preenche→remover). Zero migration/RLS/IA. Falta só push+prod pra fechar ✅. [spec](specs/R-05-orto-lancamento-manual.md) | P |
