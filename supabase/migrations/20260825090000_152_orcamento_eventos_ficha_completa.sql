-- R-130 — um orçamento de ficha inclui serviços clínicos planejados ou realizados.
-- Eventos pré-existentes são histórico e nunca cobráveis; `orcamento_eventos.evento_id`
-- continua único, portanto um evento não pode entrar em dois orçamentos.
...
begin
  ...
  -- R-130: aceita qualquer serviço que nasceu da clínica, realizado ou planejado. A origem
  -- pré-existente é somente histórico e fica fora. A unique(evento_id) fecha a corrida.
  for v_item in select value from jsonb_array_elements(p_itens)
  loop
    for v_evento_id in
